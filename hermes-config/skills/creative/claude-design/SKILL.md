...
For multi-variant e-commerce mockups (different industry themes for the same store concept), each variant must have a distinctly different visual identity — not just color swaps. See `references/ecommerce-multi-variant-mockups.md` for the full pattern including theme comparison tables, PDF extraction workflow, and content guidelines.
...
- Do not use the same card style across multi-variant mockups — each theme must feel distinct.
- Do not use Lorem Ipsum — use realistic product names, prices, and Indonesian context for ID users.

## Reading Proposal PDFs

Users often submit design briefs as PDFs. The `read_file` tool returns binary garbage for PDFs. Use:

```bash
pdftotext /path/to/file.pdf - | head -200
```

If `pdftotext` is not installed: `apt-get install -y poppler-utils`

For the full document, pipe to a file or use `less`:
```bash
pdftotext /path/to/file.pdf /tmp/proposal.txt && read_file(path="/tmp/proposal.txt")
```
